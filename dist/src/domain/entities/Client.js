...
export class Client {
    ...
    activate() {
        if (this.status === "ACTIVE") {
            throw new BusinessRuleError("Client cannot be activated from current state");
        }
        this.status = "ACTIVE";
    }
    markWarning() {
    ...
    suspend() {
        if (this.status !== "ACTIVE" && this.status !== "WARNING") {
            throw new BusinessRuleError("Only active clients can be suspend");
        }
    ...
    updateData(name, nickname, address, phone, planId, routerSerial) {
        this.name = name;
        this.nickname = nickname;
        this.address = address;
        this.phone = phone;
